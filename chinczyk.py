...
def main(genomes, config):
    playing_bu = []
    data = [["r", 0], ["g", 1], ["b", 0], ["y", 1]]

    for _, g in genomes:
        net = neat.nn.FeedForwardNetwork.create(g, config)
        g.fitness = 0
        playing_bu.append(Agent(data[0][0], data[0][1], g, net))
        data = data[1:]
    ...
    for _ in range(10):
        ...
        while not end:
            ...
            while again:
                ...
                strikes, again, chosen = playing[i].move(strikes, pawns)
                ...
                status = [pawn.finished for pawn in playing[i].pawns]
                if all(status):
                    finished.append(playing[i])
                    playing = playing[:i] + playing[i+1:]
                    again = False
