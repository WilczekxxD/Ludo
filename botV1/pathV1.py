...
class Path:
    ...
    # dummy conflict finder for creating states
    def find_conflictsV1(self, chosen):
        ...
        if not field.star or chosen.finishing == 0:
            ...
            if defence <= attack and defence != 0:
                conflict = True
                # instead of interacting with pawn objects the potential casualties list
                # is returned for further processing
                # for pawn in potential_casualties:
                #    pawn.reset()

        return conflict, potential_casualties
